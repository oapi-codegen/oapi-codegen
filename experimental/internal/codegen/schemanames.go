...
func resolveCollisions(schemas []*SchemaDescriptor, candidates map[*SchemaDescriptor]string, converter *NameConverter) {
	...
	for iteration := range maxIterations {
		...
		for _, group := range byName {
			...
			// On last iteration, just add numeric suffixes
...
// disambiguateName adds more context to make a name unique.
func disambiguateName(s *SchemaDescriptor, currentName string, index int, converter *NameConverter) string {
	path := s.Path

	// Try to add more path context based on what's in the path
	// but not already in the name

	// Check for content type differentiation
	for i, part := range path {
		if part == "content" && i+1 < len(path) {
			contentType := path[i+1]
			var suffix string
			switch {
	...
	// Check for status code differentiation (for responses)
	for i, part := range path {
		if part == "responses" && i+1 < len(path) {
			code := path[i+1]
	...
	// Check for parameter index differentiation
	for i, part := range path {
		if part == "parameters" && i+1 < len(path) {
			idx := path[i+1]
			if !strings.HasSuffix(currentName, idx) {
				return currentName + idx
			}
		}
	}

	// Check for composition type differentiation
