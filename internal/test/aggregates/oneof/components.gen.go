...
// adoptUnion reconciles OneOfObject10's own fields with union data that From* or
...
func (t *OneOfObject10) adoptUnion(b json.RawMessage) {
	object := make(map[string]json.RawMessage)
	if json.Unmarshal(b, &object) != nil {
		return
	}
	if raw, found := object["one"]; found && reflect.ValueOf(t.One).IsZero() {
		_ = json.Unmarshal(raw, &t.One)
	}
	if raw, found := object["three"]; found && reflect.ValueOf(t.Three).IsZero() {
		_ = json.Unmarshal(raw, &t.Three)
	}
	if raw, found := object["two"]; found && reflect.ValueOf(t.Two).IsZero() {
		_ = json.Unmarshal(raw, &t.Two)
	}
}
...
// boolean number is taken as one; anything else is a string.
...
func (t *OneOfObject11_AdditionalProperties) Bind(src string) error {
...
// adoptUnion reconciles OneOfObject13's own fields with union data that From* or
// Merge* just set, for the keys b defines. A property that is still unset takes the
// union's value, rather than overwriting it with a zero value when marshaling, and a
// matching additional property is dropped, so that an older copy of the key cannot
// shadow the new union data.
func (t *OneOfObject13) adoptUnion(b json.RawMessage) {
	object := make(map[string]json.RawMessage)
	if json.Unmarshal(b, &object) != nil {
		return
	}
	if raw, found := object["type"]; found && reflect.ValueOf(t.Type).IsZero() {
...
// adoptUnion reconciles OneOfObject4's own fields with union data that From* or
// Merge* just set, for the keys b defines. A property that is still unset takes the
// union's value, rather than overwriting it with a zero value when marshaling, and a
// matching additional property is dropped, so that an older copy of the key cannot
// shadow the new union data.
func (t *OneOfObject4) adoptUnion(b json.RawMessage) {
	object := make(map[string]json.RawMessage)
	if json.Unmarshal(b, &object) != nil {
		return
	}
	if raw, found := object["fixedProperty"]; found && reflect.ValueOf(t.FixedProperty).IsZero() {
		_ = json.Unmarshal(raw, &t.FixedProperty)
	}
}
...
// adoptUnion reconciles OneOfObject8's own fields with union data that From* or
// Merge* just set, for the keys b defines. A property that is still unset takes the
// union's value, rather than overwriting it with a zero value when marshaling, and a
// matching additional property is dropped, so that an older copy of the key cannot
// shadow the new union data.
func (t *OneOfObject8) adoptUnion(b json.RawMessage) {
	object := make(map[string]json.RawMessage)
	if json.Unmarshal(b, &object) != nil {
		return
	}
	if raw, found := object["fixed"]; found && reflect.ValueOf(t.Fixed).IsZero() {
		_ = json.Unmarshal(raw, &t.Fixed)
	}
}
...
// adoptUnion reconciles OneOfObject9's own fields with union data that From* or
// Merge* just set, for the keys b defines. A property that is still unset takes the
// union's value, rather than overwriting it with a zero value when marshaling, and a
// matching additional property is dropped, so that an older copy of the key cannot
// shadow the new union data.
func (t *OneOfObject9) adoptUnion(b json.RawMessage) {
	object := make(map[string]json.RawMessage)
	if json.Unmarshal(b, &object) != nil {
		return
	}
