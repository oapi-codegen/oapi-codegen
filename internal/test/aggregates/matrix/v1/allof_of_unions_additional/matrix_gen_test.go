...
// unchanged.
var samples = []string{
	"{\"card\": \"4111\", \"address\": \"1 Main St\", \"note\": \"leave at door\"}",
...
func (server) BodyHeaders(_ context.Context, req BodyHeadersRequestObject) (BodyHeadersResponseObject, error) {
	body, err := echo[Subject](req.Body)
	return BodyHeaders200JSONResponse{Body: body, Headers: BodyHeaders200ResponseHeaders{XShape: "allof_of_unions_additional"}}, err
}
...
func TestMatrixSubject(t *testing.T) {
	t.Skip("v1 embeds both unions, whose MarshalJSON and UnmarshalJSON then cancel out.")
...
func TestMatrixBodyHeaders(t *testing.T) {
	...
	for i, sample := range samples {
		t.Run(sampleName(i, sample), func(t *testing.T) {
			...
			require.NoError(t, err)
			assert.Equal(t, "allof_of_unions_additional", resp.HTTPResponse.Header.Get("X-Shape"))
			check(t, sample, resp.StatusCode(), resp.Body, resp.JSON200)
		})
	}
}
