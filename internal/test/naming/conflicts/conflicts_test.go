...
// ---------------------------------------------------------------------------
// Tests ported from name_conflict_resolution
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Tests ported from issues/issue-grab_import_names
// ---------------------------------------------------------------------------

// TestLineComments verifies that package names appearing in parameter
// description text (e.g. "base64. bytes. chi. context. echo.") are NOT
// mistakenly treated as import names by the code generator, and do not
// cause a spurious import of "openapi_types" (or any other package name
// that only appears in a field comment, not in actual usage).
//
// The assertion is that the generated code does not contain the string
// `"openapi_types"` as an import path.
func TestLineComments(t *testing.T) {
	swagger, err := openapi3.NewLoader().LoadFromFile("spec_importnames.yaml")
	require.NoError(t, err)

	opts := codegen.Configuration{
		PackageName: "namingconflicts",
