...
func RegisterHandlerVersionsWithOptions(
	...
	apiHandlers[fmt.Sprintf("/%s/json", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/json", version): wrapper.JSONExample,
	}
	apiHandlers[fmt.Sprintf("/%s/json", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/json", fibermid.LatestVersion): wrapper.JSONExample,
	}

	apiHandlers[fmt.Sprintf("/%s/multipart", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/multipart", version): wrapper.MultipartExample,
	}
	apiHandlers[fmt.Sprintf("/%s/multipart", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/multipart", fibermid.LatestVersion): wrapper.MultipartExample,
	}

	apiHandlers[fmt.Sprintf("/%s/multipart-related", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/multipart-related", version): wrapper.MultipartRelatedExample,
	}
	apiHandlers[fmt.Sprintf("/%s/multipart-related", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/multipart-related", fibermid.LatestVersion): wrapper.MultipartRelatedExample,
	}

	apiHandlers[fmt.Sprintf("/%s/multiple", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/multiple", version): wrapper.MultipleRequestAndResponseTypes,
	}
	apiHandlers[fmt.Sprintf("/%s/multiple", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/multiple", fibermid.LatestVersion): wrapper.MultipleRequestAndResponseTypes,
	}

	apiHandlers[fmt.Sprintf("/%s/reserved-go-keyword-parameters/{type}", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/reserved-go-keyword-parameters/{type}", version): wrapper.ReservedGoKeywordParameters,
	}
	apiHandlers[fmt.Sprintf("/%s/reserved-go-keyword-parameters/{type}", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/reserved-go-keyword-parameters/{type}", fibermid.LatestVersion): wrapper.ReservedGoKeywordParameters,
	}

	apiHandlers[fmt.Sprintf("/%s/reusable-responses", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/reusable-responses", version): wrapper.ReusableResponses,
	}
	apiHandlers[fmt.Sprintf("/%s/reusable-responses", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/reusable-responses", fibermid.LatestVersion): wrapper.ReusableResponses,
	}

	apiHandlers[fmt.Sprintf("/%s/text", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/text", version): wrapper.TextExample,
	}
	apiHandlers[fmt.Sprintf("/%s/text", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/text", fibermid.LatestVersion): wrapper.TextExample,
	}

	apiHandlers[fmt.Sprintf("/%s/unknown", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/unknown", version): wrapper.UnknownExample,
	}
	apiHandlers[fmt.Sprintf("/%s/unknown", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/unknown", fibermid.LatestVersion): wrapper.UnknownExample,
	}

	apiHandlers[fmt.Sprintf("/%s/unspecified-content-type", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/unspecified-content-type", version): wrapper.UnspecifiedContentType,
	}
	apiHandlers[fmt.Sprintf("/%s/unspecified-content-type", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/unspecified-content-type", fibermid.LatestVersion): wrapper.UnspecifiedContentType,
	}

	apiHandlers[fmt.Sprintf("/%s/urlencoded", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/urlencoded", version): wrapper.URLEncodedExample,
	}
	apiHandlers[fmt.Sprintf("/%s/urlencoded", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/urlencoded", fibermid.LatestVersion): wrapper.URLEncodedExample,
	}

	apiHandlers[fmt.Sprintf("/%s/with-headers", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/with-headers", version): wrapper.HeadersExample,
	}
	apiHandlers[fmt.Sprintf("/%s/with-headers", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/with-headers", fibermid.LatestVersion): wrapper.HeadersExample,
	}

	apiHandlers[fmt.Sprintf("/%s/with-union", version)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/with-union", version): wrapper.UnionExample,
	}
	apiHandlers[fmt.Sprintf("/%s/with-union", fibermid.LatestVersion)] = map[string]fiber.Handler{
		fmt.Sprintf("/%s/with-union", fibermid.LatestVersion): wrapper.UnionExample,
	}
