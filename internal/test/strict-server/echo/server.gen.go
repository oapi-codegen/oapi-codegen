...
type ServerInterface interface {
	...
	UnionInRefExample(ctx echo.Context) error
}
...
// UnionInRefExample converts echo context to params.
func (w *ServerInterfaceWrapper) UnionInRefExample(ctx echo.Context) error {
	var err error

	// Invoke the callback with all the unmarshaled arguments
	err = w.Handler.UnionInRefExample(ctx)
	return err
}
...
func RegisterHandlersWithBaseURL(router EchoRouter, si ServerInterface, baseURL string) {
	...
	router.POST(baseURL+"/with-union-in-ref", wrapper.UnionInRefExample)
...
type UnionInRefExampleResponseObject interface {
	VisitUnionInRefExampleResponse(w http.ResponseWriter) error
}

type UnionInRefExample200JSONResponse OneOfExample

func (response UnionInRefExample200JSONResponse) VisitUnionInRefExampleResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
func (sh *strictHandler) UnionInRefExample(ctx echo.Context) error {
	...
	handler := func(ctx echo.Context, request interface{}) (interface{}, error) {
		return sh.ssi.UnionInRefExample(ctx.Request().Context(), request.(UnionInRefExampleRequestObject))
	}
	for _, middleware := range sh.middlewares {
	...
	if err != nil {
		...
		return validResponse.VisitUnionInRefExampleResponse(ctx.Response())
	} else if response != nil {
		return fmt.Errorf("unexpected response type: %T", response)
	}
	return nil
