...
// TestServerLabelPathParams tests server-side deserialization of label-style path parameters.
//
// Label encoding (RFC 6570 §3.2.5):
//
//	noExplode primitive: .5
//	noExplode array:     .3,4,5
//	noExplode object:    .role,admin,firstName,Alex
//	explode primitive:   .5
//	explode array:       .3.4.5
//	explode object:      .role=admin.firstName=Alex
func TestServerLabelPathParams(t *testing.T) {
	ts, e, _ := setup(t)
	...
	t.Run("noExplode/primitive", func(t *testing.T) {
		t.Skip("runtime bug: BindStyledParameterWithOptions does not strip '.' prefix for label-style primitives")
...
// TestClientLabelPathParams tests client serialization -> server deserialization round-trip.
func TestClientLabelPathParams(t *testing.T) {
	ts, e, server := setup(t)
