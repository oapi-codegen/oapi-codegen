...
func (s strictServerInterface) TestExtMultipart(ctx context.Context, request pkg1.TestExtMultipartRequestObject) (pkg1.TestExtMultipartResponseObject, error) {
	return pkg1.TestExtMultipartdefaultMultipartResponse{
		Body: func(writer *multipart.Writer) error {
			if p, err := writer.CreatePart(textproto.MIMEHeader{"Content-Type": []string{"application/json"}}); err != nil {
				return err
			} else {
...
func TestExtMultipart(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-multipart", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtMultipartWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	mediaType, params, err := mime.ParseMediaType(res.HTTPResponse.Header.Get("Content-Type"))
	if assert.NoError(t, err) {
...
func (s strictServerInterface) TestExtOther(ctx context.Context, request pkg1.TestExtOtherRequestObject) (pkg1.TestExtOtherResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtOtherdefaultApplicationtestResponse{
		Body:          bytes.NewReader(buf),
		StatusCode:    200,
		ContentLength: int64(len(buf)),
	}, nil
...
func TestExtOther(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-other", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtOtherWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "application/test", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, []byte("bar"), res.Body)
...
func (s strictServerInterface) TestExtWildcard(ctx context.Context, request pkg1.TestExtWildcardRequestObject) (pkg1.TestExtWildcardResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtWildcarddefaultApplicationResponse{
		Body:          bytes.NewReader(buf),
		StatusCode:    200,
		ContentType:   "application/baz",
		ContentLength: int64(len(buf)),
...
func TestExtWildcard(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-wildcard", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtWildcardWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "application/baz", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, []byte("bar"), res.Body)
...
func (s strictServerInterface) TestExtNoContent(ctx context.Context, request pkg1.TestExtNoContentRequestObject) (pkg1.TestExtNoContentResponseObject, error) {
	return pkg1.TestExtNoContentdefaultResponse{
		StatusCode: 204,
	}, nil
}

func TestExtNoContent(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-nocontent", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtNoContentWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 204, res.StatusCode())
	assert.Equal(t, "", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, []byte{}, res.Body)
...
func (s strictServerInterface) TestExtFixedSpecialJSON(ctx context.Context, request pkg1.TestExtFixedSpecialJSONRequestObject) (pkg1.TestExtFixedSpecialJSONResponseObject, error) {
	return pkg1.TestExtFixedSpecialJSON200ApplicationTestPlusJSONResponse{pkg2.TestRespExtFixedSpecialJSONApplicationTestPlusJSONResponse(pkg2.TestSchema{
		Field1: "bar",
		Field2: 456,
	})}, nil
}

func TestExtFixedSpecialJSON(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-fixed-special-json", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtFixedSpecialJSONWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "application/test+json", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, &pkg2.TestSchema{
...
func (s strictServerInterface) TestExtFixedMultipart(ctx context.Context, request pkg1.TestExtFixedMultipartRequestObject) (pkg1.TestExtFixedMultipartResponseObject, error) {
	return pkg1.TestExtFixedMultipart200MultipartResponse(pkg2.TestRespExtFixedMultipartMultipartResponse(func(writer *multipart.Writer) error {
		if p, err := writer.CreatePart(textproto.MIMEHeader{"Content-Type": []string{"application/json"}}); err != nil {
			return err
		} else {
			return json.NewEncoder(p).Encode(pkg2.TestSchema{
...
func TestExtFixedMultipart(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-fixed-multipart", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtFixedMultipartWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	mediaType, params, err := mime.ParseMediaType(res.HTTPResponse.Header.Get("Content-Type"))
	if assert.NoError(t, err) {
...
func (s strictServerInterface) TestExtFixedOther(ctx context.Context, request pkg1.TestExtFixedOtherRequestObject) (pkg1.TestExtFixedOtherResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtFixedOther200ApplicationtestResponse{pkg2.TestRespExtFixedOtherApplicationtestResponse{
		Body:          bytes.NewReader(buf),
		ContentLength: int64(len(buf)),
	}}, nil
}

func TestExtFixedOther(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-fixed-other", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtFixedOtherWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "application/test", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, []byte("bar"), res.Body)
...
func (s strictServerInterface) TestExtFixedWildcard(ctx context.Context, request pkg1.TestExtFixedWildcardRequestObject) (pkg1.TestExtFixedWildcardResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtFixedWildcard200ApplicationResponse{pkg2.TestRespExtFixedWildcardApplicationResponse{
		Body:          bytes.NewReader(buf),
		ContentType:   "application/baz",
		ContentLength: int64(len(buf)),
	}}, nil
...
func TestExtFixedWildcard(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-fixed-wildcard", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtFixedWildcardWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "application/baz", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, []byte("bar"), res.Body)
...
func (s strictServerInterface) TestExtFixedNoContent(ctx context.Context, request pkg1.TestExtFixedNoContentRequestObject) (pkg1.TestExtFixedNoContentResponseObject, error) {
	return pkg1.TestExtFixedNoContent204Response(pkg2.TestRespExtFixedNoContentResponse{}), nil
}

func TestExtFixedNoContent(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-fixed-nocontent", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtFixedNoContentWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 204, res.StatusCode())
	assert.Equal(t, "", res.HTTPResponse.Header.Get("Content-Type"))
	assert.Equal(t, []byte{}, res.Body)
...
func (s strictServerInterface) TestExtHeaderMultipart(ctx context.Context, request pkg1.TestExtHeaderMultipartRequestObject) (pkg1.TestExtHeaderMultipartResponseObject, error) {
	return pkg1.TestExtHeaderMultipartdefaultMultipartResponse{
		Body: func(writer *multipart.Writer) error {
			if p, err := writer.CreatePart(textproto.MIMEHeader{"Content-Type": []string{"application/json"}}); err != nil {
				return err
			} else {
		...
		Headers: pkg2.TestRespExtHeaderMultipartResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
		StatusCode: 200,
...
func TestExtHeaderMultipart(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-multipart", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderMultipartWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderOther(ctx context.Context, request pkg1.TestExtHeaderOtherRequestObject) (pkg1.TestExtHeaderOtherResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtHeaderOtherdefaultApplicationtestResponse{
		Body: bytes.NewReader(buf),
		Headers: pkg2.TestRespExtHeaderOtherResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
		StatusCode:    200,
...
func TestExtHeaderOther(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-other", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderOtherWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderWildcard(ctx context.Context, request pkg1.TestExtHeaderWildcardRequestObject) (pkg1.TestExtHeaderWildcardResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtHeaderWildcarddefaultApplicationResponse{
		Body: bytes.NewReader(buf),
		Headers: pkg2.TestRespExtHeaderWildcardResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
		StatusCode:    200,
...
func TestExtHeaderWildcard(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-wildcard", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderWildcardWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderNoContent(ctx context.Context, request pkg1.TestExtHeaderNoContentRequestObject) (pkg1.TestExtHeaderNoContentResponseObject, error) {
	return pkg1.TestExtHeaderNoContentdefaultResponse{
		Headers: pkg2.TestRespExtHeaderNoContentResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
		StatusCode: 204,
...
func TestExtHeaderNoContent(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-nocontent", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderNoContentWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 204, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderFixedJSON(ctx context.Context, request pkg1.TestExtHeaderFixedJSONRequestObject) (pkg1.TestExtHeaderFixedJSONResponseObject, error) {
	return pkg1.TestExtHeaderFixedJSON200JSONResponse{pkg2.TestRespExtHeaderFixedJSONJSONResponse{
		Body: pkg2.TestSchema{
			Field1: "bar",
			Field2: 456,
		},
		Headers: pkg2.TestRespExtHeaderFixedJSONResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
	}}, nil
...
func TestExtHeaderFixedJSON(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-fixed-json", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderFixedJSONWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderFixedSpecialJSON(ctx context.Context, request pkg1.TestExtHeaderFixedSpecialJSONRequestObject) (pkg1.TestExtHeaderFixedSpecialJSONResponseObject, error) {
	return pkg1.TestExtHeaderFixedSpecialJSON200ApplicationTestPlusJSONResponse{pkg2.TestRespExtHeaderFixedSpecialJSONApplicationTestPlusJSONResponse{
		Body: pkg2.TestSchema{
			Field1: "bar",
			Field2: 456,
		},
		Headers: pkg2.TestRespExtHeaderFixedSpecialJSONResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
	}}, nil
...
func TestExtHeaderFixedSpecialJSON(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-fixed-special-json", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderFixedSpecialJSONWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderFixedMultipart(ctx context.Context, request pkg1.TestExtHeaderFixedMultipartRequestObject) (pkg1.TestExtHeaderFixedMultipartResponseObject, error) {
	return pkg1.TestExtHeaderFixedMultipart200MultipartResponse{pkg2.TestRespExtHeaderFixedMultipartMultipartResponse{
		Body: func(writer *multipart.Writer) error {
			if p, err := writer.CreatePart(textproto.MIMEHeader{"Content-Type": []string{"application/json"}}); err != nil {
				return err
			} else {
		...
		Headers: pkg2.TestRespExtHeaderFixedMultipartResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
	}}, nil
...
func TestExtHeaderFixedMultipart(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-fixed-multipart", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderFixedMultipartWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderFixedOther(ctx context.Context, request pkg1.TestExtHeaderFixedOtherRequestObject) (pkg1.TestExtHeaderFixedOtherResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtHeaderFixedOther200ApplicationtestResponse{pkg2.TestRespExtHeaderFixedOtherApplicationtestResponse{
		Body: bytes.NewReader(buf),
		Headers: pkg2.TestRespExtHeaderFixedOtherResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
		ContentLength: int64(len(buf)),
...
func TestExtHeaderFixedOther(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-fixed-other", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderFixedOtherWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderFixedWildcard(ctx context.Context, request pkg1.TestExtHeaderFixedWildcardRequestObject) (pkg1.TestExtHeaderFixedWildcardResponseObject, error) {
	buf := []byte("bar")
	return pkg1.TestExtHeaderFixedWildcard200ApplicationResponse{pkg2.TestRespExtHeaderFixedWildcardApplicationResponse{
		Body: bytes.NewReader(buf),
		Headers: pkg2.TestRespExtHeaderFixedWildcardResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
		ContentType:   "application/baz",
...
func TestExtHeaderFixedWildcard(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-fixed-wildcard", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderFixedWildcardWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 200, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
...
func (s strictServerInterface) TestExtHeaderFixedNoContent(ctx context.Context, request pkg1.TestExtHeaderFixedNoContentRequestObject) (pkg1.TestExtHeaderFixedNoContentResponseObject, error) {
	return pkg1.TestExtHeaderFixedNoContent204Response(pkg2.TestRespExtHeaderFixedNoContentResponse{
		Headers: pkg2.TestRespExtHeaderFixedNoContentResponseHeaders{
			Header1: "foo",
			Header2: 123,
		},
	}), nil
...
func TestExtHeaderFixedNoContent(t *testing.T) {
	hh := pkg1.Handler(pkg1.NewStrictHandler(strictServerInterface{}, nil))

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !assert.Equal(t, "/test-ext-header-fixed-nocontent", r.URL.Path) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		hh.ServeHTTP(w, r)
	...
	c, err := pkg1.NewClientWithResponses(ts.URL)
	assert.NoError(t, err)
	res, err := c.TestExtHeaderFixedNoContentWithResponse(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 204, res.StatusCode())
	assert.Equal(t, "foo", res.HTTPResponse.Header.Get("header1"))
	assert.Equal(t, "123", res.HTTPResponse.Header.Get("header2"))
