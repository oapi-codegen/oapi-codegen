...
func doGet(t *testing.T, mux *fiber.App, rawURL string) *httptest.ResponseRecorder {
...
func doPost(t *testing.T, mux *fiber.App, rawURL string, jsonBody interface{}) *httptest.ResponseRecorder {
	u, err := url.Parse(rawURL)
	if err != nil {
		t.Fatalf("Invalid url: %s", rawURL)
	}
...
func TestOapiRequestValidatorWithOptionsMultiError(t *testing.T) {
	...
	// get called.
	r.Get("/multiparamresource", func(c *fiber.Ctx) error {
		called = true
		return nil
	})
...
func TestOapiRequestValidatorWithOptions(t *testing.T) {
	...
	r.Get("/protected_resource", func(c *fiber.Ctx) error {
		called = true
	...
	r.Get("/protected_resource2", func(c *fiber.Ctx) error {
		called = true
		c.Status(http.StatusNoContent)
		return nil
	})
	...
	r.Get("/protected_resource_401", func(c *fiber.Ctx) error {
		called = true
		c.Status(http.StatusNoContent)
		return nil
	})
...
func testRequestValidatorBasicFunctions(t *testing.T, r *fiber.App) {

	called := false

	// Install a request handler for /resource. We want to make sure it doesn't
	// get called.
	r.Get("/resource", func(*fiber.Ctx) error {
		called = true
		return nil
	})
	...
	r.Post("/resource", func(c *fiber.Ctx) error {
		called = true
		c.Status(http.StatusNoContent)
		return nil
	})
