...
func Generate(swagger *openapi3.T, packageName string, opts Options) (string, error) {
	...
	if opts.GenerateTypes {
		typeDefinitions, err = GenerateTypeDefinitions(t, swagger, ops, opts.ExcludeSchemas, opts.TypesPackage)
		if err != nil {
...
func GenerateTypeDefinitions(t *template.Template, swagger *openapi3.T, ops []OperationDefinition, excludeSchemas []string, typesPackage string) (string, error) {
	schemaTypes, err := GenerateTypesForSchemas(t, swagger.Components.Schemas, excludeSchemas, typesPackage)
	if err != nil {
		return "", fmt.Errorf("error generating Go types for component schemas: %w", err)
	}

	paramTypes, err := GenerateTypesForParameters(t, swagger.Components.Parameters, typesPackage)
	if err != nil {
		return "", fmt.Errorf("error generating Go types for component parameters: %w", err)
	}
	allTypes := append(schemaTypes, paramTypes...)

	responseTypes, err := GenerateTypesForResponses(t, swagger.Components.Responses, typesPackage)
	if err != nil {
		return "", fmt.Errorf("error generating Go types for component responses: %w", err)
	}
	allTypes = append(allTypes, responseTypes...)

	bodyTypes, err := GenerateTypesForRequestBodies(t, swagger.Components.RequestBodies, typesPackage)
	if err != nil {
...
func GenerateTypesForSchemas(t *template.Template, schemas map[string]*openapi3.SchemaRef, excludeSchemas []string, typesPackage string) ([]TypeDefinition, error) {
	...
	for _, schemaName := range SortedSchemaKeys(schemas) {
		...
		goSchema, err := GenerateGoSchema(schemaRef, []string{schemaName}, typesPackage)
		if err != nil {
...
func GenerateTypesForParameters(t *template.Template, params map[string]*openapi3.ParameterRef, typesPackage string) ([]TypeDefinition, error) {
	var types []TypeDefinition
	for _, paramName := range SortedParameterKeys(params) {
		paramOrRef := params[paramName]

		goType, err := paramToGoType(paramOrRef.Value, nil, typesPackage)
		if err != nil {
...
func GenerateTypesForResponses(t *template.Template, responses openapi3.Responses, typesPackage string) ([]TypeDefinition, error) {
	var types []TypeDefinition
	...
	for _, responseName := range SortedResponsesKeys(responses) {
		...
		if found {
			goType, err := GenerateGoSchema(jsonResponse.Schema, []string{responseName}, typesPackage)
			if err != nil {
...
func GenerateTypesForRequestBodies(t *template.Template, bodies map[string]*openapi3.RequestBodyRef, typesPackage string) ([]TypeDefinition, error) {
	var types []TypeDefinition
	...
	for _, bodyName := range SortedRequestBodyKeys(bodies) {
		...
		if found {
			goType, err := GenerateGoSchema(jsonBody.Schema, []string{bodyName}, typesPackage)
			if err != nil {
