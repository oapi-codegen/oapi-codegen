...
func generateUnionsV3(ctx genContext, outSchema *Schema, schema *openapi3.Schema, path []string) error {
	...
	case len(components) == 1:
		if err := generateUnionV3(ctx, outSchema, components[0].branches, components[0].discriminator, path, true); err != nil {
			return fmt.Errorf("error generating type for oneOf: %w", err)
		}
	case schema.AnyOf != nil:
		if err := generateUnionV3(ctx, outSchema, schema.AnyOf, schema.Discriminator, path, true); err != nil {
			return fmt.Errorf("error generating type for anyOf: %w", err)
		}
	case schema.OneOf != nil:
		if err := generateUnionV3(ctx, outSchema, schema.OneOf, schema.Discriminator, path, true); err != nil {
			return fmt.Errorf("error generating type for oneOf: %w", err)
		}
	}
	return nil
...
func generateUnionComponentsV3(ctx genContext, outSchema *Schema, components []unionComponent, path []string) error {
	...
	for k, c := range components {
		for _, b := range c.branches {
			if b != nil && b.Value != nil && !isOpaqueSchema(b) {
				declared[k] = append(declared[k], propertyNames(b.Value, 0)...)
			}
		}
	...
	for k, c := range components {
		...
		before := len(outSchema.UnionElements)
		if err := generateUnionV3(ctx, outSchema, c.branches, discriminator, append(slices.Clone(path), kind), false); err != nil {
			return fmt.Errorf("error generating type for %s: %w", strings.ToLower(kind[:1])+kind[1:], err)
		}
		for _, e := range outSchema.UnionElements[before:] {
			outSchema.UnionOwnedKeys[e] = owned[k]
		}
		if c.ref != nil {
...
// alone is false when the union is one of several (see
// generateUnionComponentsV3), which then isn't collapsed into its only
// non-null branch.
func generateUnionV3(ctx genContext, outSchema *Schema, elements openapi3.SchemaRefs, discriminator *openapi3.Discriminator, path []string, alone bool) error {
