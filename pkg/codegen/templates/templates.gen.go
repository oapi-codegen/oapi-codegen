...
// ({{.Method}} {{.Path}})
{{.OperationId}}(ctx context.Context{{genParamArgs .PathParams}}{{if .RequiresParamObject}}, params {{.OperationId}}Params{{end}})
{{end}}
}
`,
...
{{end}}
  ...
  {{end}}
    ...
    {{end}}
  {{end}}
  siw.Handler.{{.OperationId}}(ctx{{genParamNames .PathParams}}{{if .RequiresParamObject}}, params{{end}})
}
{{end}}
