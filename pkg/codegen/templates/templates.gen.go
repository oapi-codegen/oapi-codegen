...
r.{{.Method | lower | title }}(options.BaseURL+"{{.Path | openAPIUriToChiUri}}", wrapper.{{.OperationId}})
})
{{end}}
return r
}
...
`,
	"inline.tmpl": `// Base64 encoded, gzipped, json marshaled OpenAPI object
...
{{end}}
{{range .}}router.{{.Method}}(baseURL + "{{.Path | openAPIUriToEchoUri}}", wrapper.{{.OperationId}})
{{end}}
}
`,
