...
type OutputOptions struct {
	...
	// ClientResponseErrorOnUnexpectedResponse makes the generated
	// `Parse<Operation>Response` functions add a `default` case to their
	// response switch that returns the sentinel `ErrUnexpectedResponse` error
	// when the server's status code and content-type match none of the
	// responses declared in the OpenAPI specification. By default no such case
	// is generated and an unexpected response yields a response object with
	// only its `Body`/`HTTPResponse` populated and a nil error.
	ClientResponseErrorOnUnexpectedResponse bool `yaml:"client-response-error-on-unexpected-response,omitempty"`
