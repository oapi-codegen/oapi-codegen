...
type Schema struct {
	...
	PackageName string // If the type is external type, this is set
...
func GenerateGoSchema(sref *openapi3.SchemaRef, path []string, typesPackage string) (Schema, error) {
	...
	if schema.AllOf != nil {
		mergedSchema, err := MergeSchemas(schema.AllOf, path, typesPackage)
		if err != nil {
	...
	if t == "" || t == "object" {
		...
		if len(schema.Properties) == 0 && !SchemaHasAdditionalProperties(schema) {
			...
			for _, pName := range SortedSchemaKeys(schema.Properties) {
				...
				pSchema, err := GenerateGoSchema(p, propertyPath, typesPackage)
				if err != nil {
			...
			if schema.AdditionalProperties != nil {
				additionalSchema, err := GenerateGoSchema(schema.AdditionalProperties, path, typesPackage)
				if err != nil {
		...
		err := resolveType(schema, path, &outSchema, typesPackage)
		if err != nil {
...
func resolveType(schema *openapi3.Schema, path []string, outSchema *Schema, typesPackage string) error {
	f := schema.Format
	t := schema.Type
	...
	case "array":
		...
		// [] in front of it.
		arrayType, err := GenerateGoSchema(schema.Items, path, typesPackage)
		if err != nil {
...
func MergeSchemas(allOf []*openapi3.SchemaRef, path []string, typesPackage string) (Schema, error) {
	...
	for _, schemaOrRef := range allOf {
		...
		schema, err := GenerateGoSchema(schemaOrRef, path, typesPackage)
		if err != nil {
	...
	var err error
	outSchema.GoType, err = GenStructFromAllOf(allOf, path, typesPackage)
	if err != nil {
...
func GenStructFromAllOf(allOf []*openapi3.SchemaRef, path []string, typesPackage string) (string, error) {
	// Start out with struct {
	objectParts := []string{"struct {"}
	for _, schemaOrRef := range allOf {
		ref := schemaOrRef.Ref
		if IsGoTypeReference(ref) {
			...
			goSchema, err := GenerateGoSchema(schemaOrRef, path, typesPackage)
			if err != nil {
				return "", err
			}
...
func paramToGoType(param *openapi3.Parameter, path []string, typesPackage string) (Schema, error) {
	...
	if param.Schema != nil {
		return GenerateGoSchema(param.Schema, path, typesPackage)
	}
	...
	return GenerateGoSchema(mt.Schema, path, typesPackage)
}
