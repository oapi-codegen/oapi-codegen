...
type Schema struct {
	...
	// UntypedEnum is set when this schema is a oneOf/anyOf of a primitive and
	// that same primitive constrained by an `enum` — i.e. "a value from this
	// enum, or any other value of the same primitive type". The Go type stays
	// the bare primitive (so any value is assignable without a conversion),
	// but EnumValues is still populated so the known values are emitted as
	// untyped constants — no distinct enum type and no Valid() method.
	// See https://github.com/oapi-codegen/oapi-codegen/issues/2412
	UntypedEnum bool
...
type EnumDefinition struct {
	...
	TypeName string
	// ConstType is the Go type assigned to the generated constants. It is
	// usually the same as TypeName, but for an "untyped enum" (see
	// Schema.UntypedEnum) it is the bare primitive type so the constants stay
	// freely assignable to/from that primitive without a conversion.
	ConstType string
	// SkipValidate suppresses generation of the Valid() method for this enum.
	// Set for untyped enums, where any value of the primitive type is legal so
	// a membership check would be misleading.
	SkipValidate bool
...
func GenerateGoSchema(sref *openapi3.SchemaRef, path []string) (Schema, error) {
	...
	// Special case: a oneOf/anyOf that expresses "a value from this enum, or
	// any other value of the same primitive type". We keep the Go type as the
	// bare primitive — so any value is assignable without a conversion — but
	// still emit the enum's known values as untyped constants.
	// See https://github.com/oapi-codegen/oapi-codegen/issues/2412
	if enumRef, ok := untypedEnumUnion(schema); ok {
		if err := oapiSchemaToGoType(enumRef.Value, path, &outSchema); err != nil {
			return Schema{}, fmt.Errorf("error resolving primitive type for untyped enum: %w", err)
		}
		outSchema.UntypedEnum = true
		outSchema.DefineViaAlias = true
		outSchema.EnumValues = buildEnumValues(enumRef.Value, combinedSchemaExtensions(enumRef), path)

		if len(path) > 1 {
			// Non-top-level: hang the constants on a named alias type and
			// reference it. Because it's an alias (`type X = string`), a field
			// of this type stays assignable to/from the bare primitive.
			typeName := SchemaNameToTypeName(PathToTypeName(path))
			if extension, ok := extensions[extGoTypeName]; ok {
				name, err := extString(extension)
				if err != nil {
	...
	if t.Slice() == nil || t.Is("object") {
		...
		outSchema.EnumValues = buildEnumValues(schema, extensions, path)
...
// buildEnumValues converts the `enum` of a primitive schema into the
// name→value map consumed by the constants template. It honors the
// x-enum-varnames / x-enumNames overrides and the OldEnumConflicts
// compatibility flag for naming.
func buildEnumValues(schema *openapi3.Schema, extensions map[string]any, path []string) map[string]string {
	...
	for _, key := range []string{extEnumVarNames, extEnumNames} {
		if extension, ok := extensions[key]; ok {
			if names, err := extParseEnumVarNames(extension); err == nil {
				enumNames = names
				break
			}
		}
	...
	values := make(map[string]string, len(sanitizedValues))
	for k, v := range sanitizedValues {
		...
		if globalState.options.Compatibility.OldEnumConflicts {
			values[SchemaNameToTypeName(PathToTypeName(append(path, enumName)))] = v
		} else {
			values[SchemaNameToTypeName(k)] = v
		}
	}
	return values
}

// untypedEnumUnion detects the "a value from this enum, or any other value of
// the same primitive type" idiom: a oneOf/anyOf with exactly two primitive
// members that share the same type and format, where one member constrains the
// value with an `enum` and the other does not. On a match it returns the
// enum-bearing member so the caller can render the bare primitive type plus the
// enum's known values as untyped constants. See issue #2412.
//
// It is deliberately strict — exactly two members, both inline primitives — so
// that anything more elaborate falls through to normal union generation rather
// than silently losing structure.
func untypedEnumUnion(schema *openapi3.Schema) (*openapi3.SchemaRef, bool) {
	var members openapi3.SchemaRefs
	switch {
	case len(schema.OneOf) > 0 && schema.AnyOf == nil:
		members = schema.OneOf
	case len(schema.AnyOf) > 0 && schema.OneOf == nil:
	...
	// Only fire on a "pure" union — a union that also carries object structure
	// is a composition we shouldn't collapse.
	if len(schema.Properties) != 0 || SchemaHasAdditionalProperties(schema) {
		return nil, false
	}

	a, b := members[0], members[1]
	// Both members must be inline primitives. A $ref names a type we should
	// preserve rather than collapse.
	if a.Ref != "" || b.Ref != "" || a.Value == nil || b.Value == nil {
		return nil, false
	}
...
// isPrimitiveOAPISchema reports whether the OpenAPI schema is one of the scalar
// primitive types (string, integer, number, boolean).
func isPrimitiveOAPISchema(s *openapi3.Schema) bool {
	t := s.Type
	return t.Is("string") || t.Is("integer") || t.Is("number") || t.Is("boolean")
}
