...
// This file holds the anyOf/oneOf half of schema-merging-behavior v2: anyOf
// and oneOf lowered into a union type with As*, From* and Merge* methods. v1
// uses it too. The allOf half is in merge_schemas_v2.go.
//
...
// generateUnionsV2 generates the anyOf and oneOf of an object schema into
// outSchema's union members.
func generateUnionsV2(ctx genContext, outSchema *Schema, schema *openapi3.Schema, path []string) error {
	...
	if schema.AnyOf != nil {
		if err := generateUnionV2(ctx, outSchema, schema.AnyOf, schema.Discriminator, anyOfPath); err != nil {
			return fmt.Errorf("error generating type for anyOf: %w", err)
		}
	}
	if schema.OneOf != nil {
		if err := generateUnionV2(ctx, outSchema, schema.OneOf, schema.Discriminator, oneOfPath); err != nil {
...
func generateUnionV2(ctx genContext, outSchema *Schema, elements openapi3.SchemaRefs, discriminator *openapi3.Discriminator, path []string) error {
	...
	// null branch into that branch (see collapseNullableUnion). We do not
	// collapse when there was no null branch (`anyOf: [{type: X}]` alone) to
	// avoid changing behavior for existing single-branch union specs that
	// may rely on the wrapper shape. The narrow condition keeps this change
	// scoped to the bug fix.
