...
// against a running server. It is used by the CLI test client to verify
// the behavior of any of the server variants.
package testclient
...
func Run(serverURL string) error {
	...
	tag := "TagOfSpot"
	addResp, err := client.AddPetWithResponse(ctx, openapi.NewPet{
		Name: "Spot",
		Tag:  &tag,
	})
	if err != nil {
	...
	tag2 := "TagOfFido"
	addResp2, err := client.AddPetWithResponse(ctx, openapi.NewPet{
		Name: "Fido",
		Tag:  &tag2,
	})
	if err != nil {
	...
	listResp, err := client.FindPetsWithResponse(ctx, &openapi.FindPetsParams{})
	if err != nil {
	...
	filterResp, err := client.FindPetsWithResponse(ctx, &openapi.FindPetsParams{Tags: &tags})
	if err != nil {
		return fmt.Errorf("FindPets failed: %w", err)
	}
	...
	emptyResp, err := client.FindPetsWithResponse(ctx, &openapi.FindPetsParams{Tags: &noTags})
	if err != nil {
		return fmt.Errorf("FindPets failed: %w", err)
	}
	...
	finalResp, err := client.FindPetsWithResponse(ctx, &openapi.FindPetsParams{})
	if err != nil {
		return fmt.Errorf("FindPets failed: %w", err)
	}
